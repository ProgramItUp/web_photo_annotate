...
1.  **Define the Goal & Context:**
    ...
    *   Mention the tools/environment (e.g., web server, browser).
2.  **Establish a Baseline:**
    *   Ensure the code is checked out to the correct branch (`git switch`).
    *   Run the application (e.g., start the local web server: `python -m http.server 8001`).
...
3.  **Initial Observation:**
    ...
    *   Note any initial errors (SyntaxError, TypeError, 404s) that appear on page load or during the basic test.
...
4.  **Formulate a Hypothesis:** Based on the errors and incorrect behavior, make an educated guess about the root cause.
...
6.  **Gather Information (Runtime Analysis):**
    *   **Logging:** If the code's behavior isn't clear, add `console.log` statements to the JavaScript:
...
7.  **Analyze Collected Data:** Compare the logged values, DOM structure (especially from Puppeteer dumps), or error messages against your hypothesis.
    *   *Example:* "The Puppeteer DOM dump shows Fabric.js added extra containers, explaining why our CSS selectors failed."
    *   *Example:* "The log shows `offsetX/Y` are relative to the `IMG`, but the CSS positions the container relative to `#image-container`. This confirms the coordinate mismatch hypothesis."
8.  **Isolate the Problem:** If multiple issues exist, focus on fixing one at a time (e.g., fix positioning *before* fixing activation logic).
9.  **Propose & Apply a Fix:** Based on the analysis, suggest a specific code change (JS logic, CSS rule, HTML attribute).
10. **Test the Fix:**
    *   **Provide Clear Test Steps:** (As you requested!) Be concise and specific.
...
11. **Repeat:** If the fix didn't work or introduced new issues, loop back to Phase 2 (Hypothesis/Investigation) or Phase 3 (Targeted Debugging). Refine the hypothesis based on the latest results.
12. **Address Related Issues:** Once the primary bug is fixed, address secondary problems or required behavior changes identified earlier (like the laser pointer activation logic).
...
20. When updating the work_log_*.md file to get the current date time and write short entries as steps are taken.  Update the logfile at the bottom so newest entries are at the top.
Use cursor or other AI coding tools, instruct them to be your assitant
21. Write yourself and others short, clear, numbered steps for testing instructions.
